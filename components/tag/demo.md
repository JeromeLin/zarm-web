    ...
    return (
      <>
        ...
        {inputVisible ? (
          ...
          <Button ref={(ele) => (this.button = ele)} onClick={this.onShowInput}>
            + New Tag
          </Button>
        )}
        <Button theme="danger" style={{ marginLeft: 10 }} onClick={this.onRemoveAll}>
          Remove All
        </Button>
      </>
    );
...
class Demo extends React.Component {
  ...
  selectItem = (nextStatus, index) => {
    ...
    this.setState(
      ...
      console.log(
        '当前选中的城市:',
        this.state.cities.filter((c) => c.checked),
      ),
    );
  };
