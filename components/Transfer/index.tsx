...
class Transfer extends Component<PropsType, any> {
  ...
  _onAdd(){
    ...
    let selected = [...this.state.initialValue].filter(item => (this.state.selectedLeft.indexOf(item[this.props.keyOfItem]) > -1) && item);
  ...
  _onMinus(){
    ...
    let selected = [...this.state.selectedValue].filter(item => (this.state.selectedRight.indexOf(item[this.props.keyOfItem]) > -1) && item);
  ...
  _onDoubleAdd(e){
    ...
    let selected = [...this.state.initialValue].filter(item => (e.target.textContent === item[this.props.displayNameOfItem]) && item);
    let selectedValue = this.state.selectedValue.concat(selected);
    this.setState({
      initialValue,
      selectedValue
  ...
  _onDoubleMinus(e){
    ...
    let selected = [...this.state.selectedValue].filter(item => (e.target.textContent === item[this.props.displayNameOfItem]) && item);
    let initialValue = this.state.initialValue.concat(selected);
    this.setState({
      initialValue,
      selectedValue
  ...
  render() {
    ...
    return (
        <div style={{ ...style, width, height }} className={cls}>
