	...
	render(){
		...
		return(
			<div style={{margin:'20px 20px 0 50px'}}>
				...
				<SliderContainer min={0} max={300} step={1} defaultValue={80} round={true}/>
				...
				<div>配合其他component显示当前value: min=40, max=600, step=5, defaultValue=180, showTip=false, solid=true</div>
				...
				<SliderContainer min={-100} max={300} step={1} 
					defaultValue={[50, 150]} 
					HandleAmount={2} 
					isRange={true}
					getValue={this.getValue3}/>
				<Button>{this.state.value3}</Button> <Button>{this.state.value4}</Button>
				<div>选择范围: min=-100, max=300, step=1, defaultValue=[50, 150], <b>isRange = true</b>, HandleAmount=2</div>
				<div><b>范围选择器需设置isRange = true 才会显示中间段的颜色</b>, (better to set defaultValue in ascending order)</div>

				<SliderContainer min={-20} max={100} step={1} 
					defaultValue={[20, 60]} 
					HandleAmount={2} 
					isRange={true}
					isPass={false}/>
					<div>min=-20, max=100, step=1, defaultValue=[20, 60]</div>
					<b>如需handle之间不越界，除了设置 isRange = true 之外还需设 isPass = false 保证handle之间不越界 </b>
				<SliderContainer 
					min={-100} 
					max={800} step={1} 
				...
				<SliderContainer max={200} defaultValue={50} theme={'info'} solid={true}/>
				theme info: max=200, min=0, defaultValue=50, step =1, solid=true

				<SliderContainer max={200} defaultValue={50} theme={'success'} round={true}/>
				theme success: max=200, min=0, defaultValue=50, step =1, round=true

				<SliderContainer max={200} defaultValue={100} theme={'warning'} solid={true} round={true}/>
				theme warning: max=200, min=0, defaultValue=100, step =1, solid=true, round=true
